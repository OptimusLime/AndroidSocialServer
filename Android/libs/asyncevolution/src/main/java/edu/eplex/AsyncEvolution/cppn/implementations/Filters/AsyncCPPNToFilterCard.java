 ...
 */
public class AsyncCPPNToFilterCard implements AsyncPhenotypeToUI<double[][], GridCard> {
...
//                Bitmap inputImage = EvolutionBitmapManager.getInstance().getBitmap(params.get("image").asText());
                ...
                {
                    double[] cppnOuts = results[c];
                    int[] rgb =  ConvertToColorRange(cppnOuts);//PicHSBtoRGB(hsv[0], clampZeroOne(hsv[1]), Math.abs(hsv[2]));
    ...
    private int[] ConvertToColorRange(double[] rgb)
    {
       return new int[]{
               (int) Math.floor(Math.abs(rgb[0])*255.0),
               (int)Math.floor(Math.abs(rgb[1])*255.0),
               (int)Math.floor(Math.abs(rgb[2])*255.0)};
    }
