package edu.eplex.AsyncEvolution.cppn.implementations;
 ...
 */
public class AsyncCPPNOutputToHomeCard implements AsyncPhenotypeToUI<double[][], HomeGridCard> {

    //need to know the activity for creating the UI
    @Override
    public Task<HomeGridCard> asyncPhenotypeToUI(final Activity a, final String wid, final double[][] phenotype, final JsonNode params) {

        return Task.callInBackground(new Callable<HomeGridCard>() {
            @Override
            public HomeGridCard call() throws Exception {
                return convertArtifactToPhenotype(a, wid, phenotype, params);
            }
        });
    }

    private HomeGridCard convertArtifactToPhenotype(Activity activity, final String wid, double[][] results, final JsonNode params)
    {
        ...
        if(activity != null)
        {
            ...
            HomeGridCard card = new HomeGridCard(activity);
